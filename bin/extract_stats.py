...
def build_payload(config: dict, key: str, time_filter: dict) -> str:
    filters = config[key]["queryObject"]["predefinedContainer"]["filters"]
...
default_out_path = 'data/current/daten_berlin_de.stats.json'

parser = argparse.ArgumentParser(description="Extract the dataset page access data from daten.berlin.de for a given month.")
...
parser.add_argument('-c', '--config', default=default_conf_path,
                    help=f"The path to the JSON config file defining the query to the Mapp Analytics API. Default is {default_conf_path}.")
...
#     ]
# ]
# the first row is the dimension I was querying for (time_year), the second row
# ("00000000") is the aggregated total. Since I queried for the year, whcih is the
# same for all traffic in any given month, the total is the same as the first row.
# I can use both.

totals_row = data['rows'][-1]
month_total = {
